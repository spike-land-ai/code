...
export const importFromString = async (code: string) => {
  ...
  // Check if we're in a real browser environment (JSDOM lacks createObjectURL)
...
async function _loadAppComponent(
  ...
  if (App) {
    ...
    if (sessionTranspiled) {
      ...
      const { data: mod, error: importErr } = await tryCatch(
        import(
          /* @vite-ignore */
          `${currentOrigin}/live/${codeSpace}/index.js`
        ),
      );
      AppToRender = importErr || !mod
        ? FallbackErrorComponent
        : mod.default as FlexibleComponentType;
    }
